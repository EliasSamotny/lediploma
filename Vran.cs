...
namespace l_application_pour_diploma{
    public partial class Vran : Form{
        ...
        private int calcrank(List<Point> set, Point point){
            ...
            if (set.Contains(new Point(point.X - 1, point.Y    ))) count++;
            if (set.Contains(new Point(point.X - 1, point.Y + 1))) count++;
            if (set.Contains(new Point(point.X    , point.Y + 1))) count++;
            if (set.Contains(new Point(point.X + 1, point.Y + 1))) count++;
            if (set.Contains(new Point(point.X + 1, point.Y    ))) count++;
            if (set.Contains(new Point(point.X + 1, point.Y - 1))) count++;
            if (set.Contains(new Point(point.X    , point.Y - 1))) count++;
        ...
        private List<List<int>> QSort(List<List<int>> set, int firstIndex = 0, int lastIndex = -1) {
            
            if (lastIndex < 0) lastIndex = set.Count - 1;
            if (firstIndex >= lastIndex)
                return set;
            
            int middleIndex = (lastIndex - firstIndex) / 2 + firstIndex, currentIndex = firstIndex;

            ((set[firstIndex][0], set[firstIndex][1]), (set[middleIndex][0], set[middleIndex][1])) = ((set[middleIndex][0], set[middleIndex][1]), (set[firstIndex][0], set[firstIndex][1]));
            for (int i = firstIndex + 1; i <= lastIndex; ++i){
                if (set[i][1] <= set[firstIndex][1]) {
                    currentIndex++;
                    ((set[currentIndex][0], set[currentIndex][1]), (set[i][0], set[i][1])) = ((set[i][0], set[i][1]), (set[currentIndex][0], set[currentIndex][1]));
                }
            }
            ((set[firstIndex][0], set[firstIndex][1]), (set[currentIndex][0], set[currentIndex][1])) = 
            ((set[currentIndex][0], set[currentIndex][1]), (set[firstIndex][0], set[firstIndex][1]));
            set = QSort(set, firstIndex, currentIndex - 1);
            set = QSort(set, currentIndex + 1, lastIndex);
            return set;
        }
